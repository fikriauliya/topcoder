...
def do_test(commands, A, B, __expected):
    startTime = time.time()
...
def run_tests():
    ...
    with open("OneDimensionalRobotEasy.sample", "r") as f:
        while True:
            label = f.readline()
            if not label.startswith("--"): break

            commands = f.readline().rstrip()
            ...
            passed += do_test(commands, A, B, __answer)

    sys.stdout.write("\nPassed : %d / %d cases\n" % (passed, cases))

    T = time.time() - 1399958413
    PT, TT = (T / 60.0, 75.0)
